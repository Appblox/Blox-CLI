...
const { appBloxLogin, appBloxAccessToken } = require('./utils/api')
...
async function getTokensFromAuthorizationCode(code) {
  ...
  try {
    res = await axios.get(
      `${appBloxAccessToken}?device_code=${code}&grant_type=urn:ietf:params:oauth:grant-type:device_code`,
      {
...
function loginWithLocalhost(port, authUrl) {
  return new Promise((resolve, reject) => {
    const server = http.createServer(async (req, res) => {
      const headers = {
        'Access-Control-Allow-Origin': 'http://shield.appblox.io',
 ...
 * @returns
 */
async function loginWithAppBlox(localhost, userHint) {
  if (localhost) {
    try {
      const port = await getPort()
      return await loginWithLocalhostAppBlox(port, userHint || null)
    } catch {
