 ...
 * else error
 */
class CustomListV2 extends ListPrompt {
  ...
  _run(cb) {
    ...
    if (self.firstRender) {
      Promise.resolve(
        self.opt.source.getPage(1).then((choices) => {
  ...
  onSubmit(value) {
    ...
    switch (value) {
      case 'LoadMore':
        return Promise.resolve(this.opt.source.nextPage()).then((choices) => {
          self.opt.choices = new Choices(choices)
          self.selected = 0
          self.render()
        })
      case 'LoadPrev':
        return Promise.resolve(this.opt.source.prevPage()).then((choices) => {
          self.opt.choices = new Choices(choices)
          self.selected = 0
          self.render()
        })
